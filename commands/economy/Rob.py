...
from Niludetsu.economy.checks import NotSelf, NotBot, CheckCooldown
...
class Rob(commands.Cog):
    ...
    @economy(
        ...
        CheckCooldown("rob", "Ты только что провернул дельце!"),
    )
    async def rob(self, ctx: commands.Context, member: discord.Member) -> None:
        ...
        if victim_wallet <= 0:
            await ctx.reply(
                embed=EconomyEmbed.error(f"У {member.display_name} в кошельке пусто. Всё лежит в банке — не подлезешь."),
                ephemeral=True,
            )
            return
        ...
        if random.random() <= ROB_SUCCESS_CHANCE:
            ...
            removed, remove_msg = await self.economy.remove_money(target_id, guild_id, steal_amount, event="rob", related_user_id=user_id)
            if not removed:
                await ctx.reply(embed=EconomyEmbed.error(remove_msg), ephemeral=True)
                return

            await self.economy.add_money(user_id, guild_id, steal_amount, share_spousal=True, event="rob", related_user_id=target_id)
            ...
            if penalty > 0:
                await self.economy.remove_money(user_id, guild_id, penalty, event="rob_penalty")
