...
class UtilitiesSystem(commands.Cog):
    ...
    async def qrdecode(
        ...
        if not ctx.message.attachments:
            t = _(ctx=ctx)
            await ctx.reply(embed=Embed.error(description=t('general', 'qrdecode_no_image')))
            return
    ...
    async def mcserver(
        ...
        if not address:
            ...
            embed = Embed.error(
                ...
                description=t('general', 'mcserver_usage')
            )
    ...
    async def ping(self, ctx: commands.Context):
        ...
        embed = Embed.default(
            title=t('general', 'ping_title'),
            description=t('general', 'ping_desc', latency=latency),
    ...
    async def hash_command(self, ctx: commands.Context, *, text: str = None):
        if not text:
            ...
            await ctx.reply(embed=Embed.error(
                description=t('general', 'hash_usage')
            ))
            return
    ...
    async def ascii_command(self, ctx: commands.Context, *, args: str = None):
        if not args:
            t = _(ctx=ctx)
            await ctx.reply(embed=Embed.error(
                description=t('general', 'ascii_usage')
            ))
            return
